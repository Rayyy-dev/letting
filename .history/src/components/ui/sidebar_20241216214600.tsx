...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="footer"
      className={cn("flex flex-col gap-1 p-2 border-t", className)}
      {...props}
    />
  )
})
...
>(({ className, isActive, asChild = false, tooltip, ...props }, ref) => {
  ...
  const button = (
    <Comp
      ...
      className={cn(
        "group/menu-button relative flex w-full select-none items-center rounded-lg px-2 py-2 text-sm outline-none transition-colors",
        ...
        "[&>div>span]:group-data-[collapsible=icon]:hidden [&>div>svg:last-child]:group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
    />
