...
export function ViewingTable() {
  ...
  const [selectedViewingType, setSelectedViewingType] = React.useState<'upcoming' | 'pending' | 'completed'>('upcoming');
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-4 border-b">
          {(['upcoming', 'pending', 'completed'] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setSelectedViewingType(tab)}
              className={cn(
                "pb-2 px-2 text-sm font-medium capitalize",
                selectedViewingType === tab 
